...
<head>
    ...
    <title>Return Requests - Librarian Portal</title>
    ...
    <style>
        ...
        .dues-badge { padding: 6px 14px; border-radius: 10px; font-weight: 700; font-size: 0.85rem; }
        .dues-danger { background: rgba(234, 84, 85, 0.1); color: #ea5455; border: 1px solid rgba(234, 84, 85, 0.2); }
        .dues-success { background: rgba(40, 199, 111, 0.1); color: #28c76f; border: 1px solid rgba(40, 199, 111, 0.2); }

        .btn-process {
            background: #2ecc71; color: white; border-radius: 12px;
            padding: 10px 22px; font-weight: 600; border: none; transition: 0.3s;
        }
        .btn-process:hover { background: #27ae60; box-shadow: 0 5px 15px rgba(46, 204, 113, 0.3); transform: scale(1.03); color: white; }
    ...
    <div id="content">
        <nav class="top-navbar d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center">
                ...
                <h5 class="fw-bold mb-0 text-dark">Return Approvals</h5>
            </div>
        ...
        <div class="text-center">
            <div class="request-nav">
                <a href="issue_requests.php" class="nav-btn">Issue Requests</a>
                <a href="renew_requests.php" class="nav-btn">Renew Requests</a>
                <a href="return_requests.php" class="nav-btn active">Return Requests</a>
            </div>
        </div>

        <div class="custom-table-card">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="fw-bold mb-0"><i class="bi bi-arrow-return-left text-primary me-2"></i> Pending Returns</h5>
                <span class="badge bg-soft-primary text-primary px-3 py-2 rounded-pill" style="background: rgba(93,99,212,0.1)">Audit Log & Dues</span>
            </div>

            <?php
            $sql = "SELECT return.BookId, return.RollNo, Title, datediff(curdate(), Due_Date) as dues 
                    FROM LMS.return, LMS.book, LMS.record 
                    WHERE return.BookId=book.BookId AND return.BookId=record.BookId AND return.RollNo=record.RollNo";
                        ...
                        <tbody>
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><code class="fw-bold text-muted">#ID-<?php echo $bid; ?></code></td>
                                    <td>
                                        <div class="fw-bold text-dark"><?php echo htmlspecialchars($title); ?></div>
                                        <small class="text-muted">Return Submission</small>
                                    </td>
                                    <td>
                                        <?php if($dues > 0): ?>
                                            <span class="dues-badge dues-danger">
                                                <i class="bi bi-exclamation-triangle-fill me-1"></i> $<?php echo $dues; ?>.00
                                            </span>
                                        <?php else: ?>
                                            <span class="dues-badge dues-success">
                                                <i class="bi bi-check-circle-fill me-1"></i> No Dues
                                            </span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="text-end">
                                        <a href="acceptreturn.php?id1=<?php echo $bid; ?>&id2=<?php echo $rno; ?>&id3=<?php echo ($dues > 0 ? $dues : 0); ?>" class="btn btn-process shadow-sm">
                                            Accept Return
                                        </a>
                                    </td>
                                </tr>
            ...
            <?php 
                ...
                echo '<div class="empty-state">
                        ...
                        <h5 class="fw-bold mt-3 text-muted">No Pending Returns</h5>
                        <p class="text-muted">Excellent! All books have been checked in successfully.</p>
                      </div>';
            }
            ?>
        </div>
